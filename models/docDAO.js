			...
			db.doc.findOne({
				...
				if (reply.type == "doc") {
					db.doc.update({
						...
						$set: {
							path: reply.path.replace(oldPath, newPath)
						}
					}, function(err, reply) {
						if (err) {
							that.innerError = true;
												...
												db.doc.findOne({
													...
													if (err) {
														...
														db.doc.update({
															...
															$set: {
																owner: rootDir.owner,
																members: rootDir.members,
																permission: rootDir.permission
															}
														}, function(err, reply) {
															if (err) {
																lock.release(rootPath);
	...
	db.doc.findOne({
		...
		//                     }
			if (result.type == "dir") {
				...
				visionLength = result.revisions.length;
				var versionId = result.revisions[visionLength - 1];
